...
```
```py
... line=dict(color="yellow"), marker=dict(color="green")  ...
```
...
sc = go.Scatter(x=np.random.rand(n), y=np.random.rand(n), mode='markers',
                marker=dict(color='blue', opacity=(1-opacity), size=50*opacity))
...
Immediately fater reading the dataframe from the file, add the following:
