...
- Out of the box, provides basic interactivity (hover over data, zoom in, etc.) and animation with sliders
...
Exporting & Next Steps
    ...
    Q&A / Mini-Challenge: Give students a fresh dataset and ask them to find one trend and highlight it using
    a custom hover label.

Focus on Hover: Spend extra time showing how to clean up hover text. It's the difference between a "techy"
chart and a professional one.
...
::: {.callout-important}
## Key point
In general, use `fig.show()` to enforce displaying the plot (shell or Jupyter). Depending on your setup, this
command might not be necessary inside a Jupyter notebook if you create a figure with `go.Figure(...)` without
saving it to a variable -- then your plot should be displayed automatically.
:::
