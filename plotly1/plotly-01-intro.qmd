...
To run a similar script from the command line on the training cluster, I need to change the last line
`fig.show()` to `fig.write_image("lines.png")` to write directly into a file, since there is no browser to
open a tab in on the remote system.

You can find more details at <https://plotly.com/python/renderers>{target="_blank"}.

## Two plotting interfaces

1. original Plotly Graph Objects library `plotly.graph_objs`
    - most capable plotting interface -- you saw its example in the previous section
...
1. newer [Plotly Express](https://plotly.com/python/plotly-express){target="_blank"} library `plotly.express`
...
and then customize it with Graph Objects controls.
