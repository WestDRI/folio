...
| State management | stateless Python backend and a stateful React.js frontend that looks for any Python callbacks that have *that component property* listed as an Input or State | stateful (server-side session management) |
...
| Client-side Python (browser) | not possible at the moment; use Plotly Express with standalone HTML5 for basic interactivity | can run Python code via WebAssembly using experimental Shinylive model: the browser becomes the server; some restrictions, including your data size |
