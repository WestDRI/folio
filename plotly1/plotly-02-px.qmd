...
In Plotly Express, with a dataframe input, you can animate in one of the columns. There are two additional
flags to pass to a plotting function:
...
Try saving this interactive animation as an HTML5 file — it works and is completely self-contained, rendering
in a browser without a backend server.
:::
...
What if you want to have multiple objects per frame, e.g. two dots on two spirals, respectively?

1. One option would to use Graph Objects where you can plot two spirals at the same time.
1. Another option is to format your data as a dataframe:
