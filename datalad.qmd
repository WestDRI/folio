...
<span style="font-size:1.8em;">https://folio.vastcloud.org/datalad</span>
...
On a Mac with [Homebrew](https://brew.sh){target="_blank"} installed:
...
[official installation guide](http://handbook.datalad.org/en/latest/intro/installation.html){target="_blank"}.
...
::: {.callout-note}
Some files in your dataset will be stored as plain files, some files will be put in the annex,
...
- `yoda`: configure a dataset according to the [yoda
  principles](https://handbook.datalad.org/en/latest/basics/101-127-yoda.html){target="_blank"}  
...
Let's use some file examples from the official [DalaLad handbook](http://handbook.datalad.org){target="_blank"}:
...
5. *if we have time:* manage multiple Git repos under one dataset

## (1) two users on a shared cluster filesystem working with the same dataset

For simplicity, let's assume both users share the same GID (group ID), i.e. they are from the same research
group. Extending this workflow to multiple users with different GISs can be done via [Access control lists
(ACLs)](https://docs.alliancecan.ca/wiki/Sharing_data#Access_control_lists_(ACLs)){target="_blank"}.
...
## (4) publish a dataset on GitHub with publicly-accessible annexed files on Nextcloud
...
- [DalaLad handbook](http://handbook.datalad.org){target="_blank"}
- [Frequently Asked Questions](https://handbook.datalad.org/en/latest/basics/101-180-FAQ.html){target="_blank"}
- [DalaLad cheatsheet](https://jadecci.github.io/notes/Datalad.html){target="_blank"}
