...
### Advanced exercise: run a VisIt visualization script
...
::: {.callout-caution collapse="true"}
##### Visualize a 3D dataset inside a container
...
1. build an Ubuntu sandbox
2. download and unpack VisIt into the sandbox
3. VisIt's CLI isn't purely headless -- it starts a viewer that needs X11 and other libraries -- so install
   these libraries with:
```
...
5. start the shell inside the container:
```sh
apptainer shell visit.sif
```
6. start VisIt's CLI:
```sh
Apptainer> ./visit3_4_2.linux-x86_64/bin/visit -cli -nowin
```
7. copy the dataset `sineEnvelope.nc` from `/project/def-sponsor00/shared/paraview.zip`
8. run the following visualization script:
```py
OpenDatabase("sineEnvelope.nc")
