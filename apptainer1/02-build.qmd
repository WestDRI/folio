...
Blue lines show workflows without modifying a container, whereas grey-line workflows modify system files in
your container and typically require root, although there are non-root workarounds in dotted boxes.
</p>
<br>
...
::: {.callout-important}
##### Key point
- `apptainer run image.sif` &nbsp; runs the container's default script
- `apptainer exec image.sif <command>` &nbsp; runs a single command
:::
...
Similarly, we can run a **Docker** container directly off Docker Hub. However, it will first convert a
...
Try running this command again, and it'll use the cached SIF image!

Let's try something more basic:

```sh
apptainer run docker://ubuntu   # wait for it build the container;
                                # at the end press Ctrl-D to exit
```

What happened here in the last example? Well, there was no default script, so it presented the container shell
to type commands. You can exit the container with Ctrl-D or `exit`. The end result is the same as if we typed
`apptainer shell docker://ubuntu` which opens an interactive shell inside the container.
...
::: {.callout-important}
##### Key point
- `apptainer shell image.sif` &nbsp; opens an interactive shell inside the container
:::

::: {.callout-caution collapse="true"}
##### Exercise: check Ubuntu version
On a Linux system the file `/etc/os-release` shows the current Linus distribution and version. Which OS are we
running on the host, and which Ubuntu version is inside the container? Find the answer in 2 ways:

1. with `apptainer run` or `apptainer shell` (which produce the same result without a default script)
2. with `apptainer exec`
:::

::: {.callout-note}
##### Runscripts from Docker → Apptainer image conversion
Docker images don't have an explicit runscript in the Apptainer sense, so when you convert a Docker image into
an Apptainer image, Apptainer translates some Docker image settings into an equivalent runscript that will be
displayed with `apptainer inspect -r image.sif`, but this is not the traditional runscript. As far as
`apptainer run` is concerned, there is no runscript.
:::
...
::: {.callout-caution collapse="true"}
##### Exercise: other inspection flags
...
- [Build a Container](https://apptainer.org/docs/user/latest/build_a_container.html){target="_blank"} page in
...
research collaborator, or the Alliance. For example, in the Alliance Federation there have been some
discussions to provide a set of Apptainer images with GPU development tools for different GPU architectures,
such as NVIDIA's CUDA and AMD's ROCm, as it is often difficult to compile these from source. If you would like
these or other custom pre-configured images, please reach out to the Alliance to request them.
...
#### <u>Build consideration #1:</u> modifying system files inside the container (root vs. regular user)
...
we provide free cloud projects to researchers for spinning up VMs.
:::

#### <u>Build consideration #2:</u> host's filesystem limitations
...
technical details of some of the issues see
[this page](https://apptainer.org/docs/admin/latest/installation.html#lustre-gpfs){target="_blank"}.
...
sudo du -sh ubuntu.dir               # 139M; sudo necessary to scan root directories
```
...
scp ubuntu.sif user01@apptainer.vastcloud.org:/project/def-sponsor00/shared
```
...
### Building a container from a definition file: root or not
...
We will bootstrap our image from a minimal Ubuntu 24.04 Linux Docker image as then run it as a regular user:
...
ls -l python.sif           # 80M
apptainer run python.sif   # Hello World! Hello from our custom Apptainer image!
```
...
as described in [the user guide](https://apptainer.org/docs/user/latest/definition_files.html#sections){target="_blank"}.
...
image to remote cloud resources. One popular cloud service for this is the
[Remote Builder](https://cloud.sylabs.io/builder){target="_blank"}
from SyLabs, the developers of Apptainer. If you want
...
## Practical Apptainer workflows

ParaView and VisIt are two scientific applications that are **quite difficult** to compile from source. We
have both installed on the production clusters -- but let's imagine that we don't, and you need to install
your own. The two scenarios below show how you can do that.

1. With ParaView, we download an off-the-shelf Docker image with a pre-compiled ParaView server.
2. With VisIt, we start with a bare-bones Ubuntu image and then download a pre-compiled VisIt for Ubuntu,
   unpack it into the sandbox, and install other required libraries.

### Run client-server ParaView from a container

ParaView is a very popular open-source scalable 3D scientific visualization tool. To visualize large datasets
on an HPC cluster, you can run ParaView in a client-server mode, with your local ParaView GUI acting as a
client, and a remote ParaView server on the cluster. Let's try to run ParaView server from inside an Apptainer
image.
...
Let's search [Docker Hub](https://hub.docker.com) for *"paraview"* or *"topologytoolkit"*. Pick a family of
images and a tag (version) and copy the command for it, e.g. the command could be `docker pull
topologytoolkit/ttk-dev:5.11.0`. Note that the URL syntax in the `apptainer pull ...`  command is different,
so you will need to edit it accordingly.

::: {.callout-note}
##### Some things to watch out for
1. Image size -- building a 5 GB image might take hours to build.
1. Image age -- avoid using outdated images, e.g. a 5-year-old ParaView build.
1. Whether or not it supports GPU.
1. Read the description: we are are looking for `pvserver` and not much else.
:::

Here are few ParaView images I tried:

- the official `docker://kitware/paraview:ci-fedora42-20250915` took 1h20m to create a 4.6G SIF image, but
  there does not seem to be any `paraview` or `pvserver` in it
- someone else's recent `docker://finback9/paraview-base:0.1` has ParaView 6.0 and took 60m to create a 818M
  SIF image, but it is parallel and requires a compatible MPI installation to run
- TTK's `docker://topologytoolkit/ttk-dev:5.11.0` took 25m to create a 779M SIF image, but its latest is 5.11
  -- **let's go with this one**

<!-- for -->
<!-- *"topologytoolkit"*. Click on the result *"topologytoolkit/ttk-dev"* and then on Tags -- you should see the -->
<!-- suggested Docker command *"docker pull topologytoolkit/ttk-dev:5.11.0"*. From Apptainer, the address will be -->
<!-- *"docker://topologytoolkit/ttk-dev:5.11.0"*. -->

The following commands require online access and will work only on Fir (and the training cluster!)  where
...
module load apptainer
salloc --time=2:0:0 --mem-per-cpu=3600
apptainer pull topologytoolkit.sif docker://topologytoolkit/ttk-dev:5.11.0   # took 25m to create a 779M SIF image, latest is 5.11
```

::: {.callout-note}
On other production clusters -- Nibi, Rorqual, Trillium, Narval -- compute nodes do not have Internet access,
so you will have to use a two-step approach there:
```sh
cd ~/scratch
wget https://raw.githubusercontent.com/moby/moby/master/contrib/download-frozen-image-v2.sh
...
Let's use this image! While still inside the job (on a compute node), copy the dataset `sineEnvelope.nc` from
`/project/def-sponsor00/shared/paraview.zip` into a `~/data` folder and then start the ParaView server:

```sh
unzip /project/def-sponsor00/shared/paraview.zip -d ~ data/sineEnvelope.nc   # extract sample data into ~/data
apptainer exec -B /home paraviewBase.sif pvserver
```

If successful, it should say something like:  
*"Accepting connection(s): node1.int.apptainer.vastcloud.org:11111"*  
-- write down the node name and the port number as you will need them in the next step.

On your laptop, set up SSH port forwarding to this compute node and port:

```sh
ssh username@apptainer.vastcloud.org -L 11111:node1:11111
```
...
We did something similar with a Python container earlier, so you can refer to the notes in this section.
...
### Run a VisIt visualization script

[VisIt](https://visit-dav.github.io/visit-website/index.html){target="_blank"} is an open-source scalable
visualization package that is notoriously difficult to compile. Fortunately, it is available as a precompiled
package for Ubuntu, so you can run it inside an Ubuntu container.

In this demo we will use VisIt to visualize a 3D dataset.

We have two options:

1. unpack VisIt inside the container &nbsp;&nbsp;<span
   style="font-size:1.2em;">⇒</span>&nbsp;&nbsp; larger container image, fewer files on the host
2. unpack VisIt into a `/scratch` directory outside the container &nbsp;&nbsp;<span
   style="font-size:1.2em;">⇒</span>&nbsp;&nbsp; smaller container image, more files on the host
   
Let's go with option (1) which will make the container self-sufficient. Here are our steps:

1. Build an Ubuntu sandbox and enter it with `--fakeroot --writable` flags:

```sh
apptainer build --sandbox ubuntu.dir docker://ubuntu
mkdir ubuntu.dir/{project,scratch}
apptainer shell --fakeroot --writable ubuntu.dir
```

2. VisIt's CLI isn't purely headless -- it starts VisIt's viewer, which needs X11 and other libraries. Let's
   install these into the sandbox:

```sh
Apptainer> apt-get -y update
Apptainer> apt-get install -y libfontconfig libx11-6 libxkbcommon0 libxcursor1 libxext6 liblapack3
Apptainer> apt-get install -y wget
```

3. Download and unpack the latest VisIt package for Ubuntu into the sandbox:

<!-- Install the [latest (3.4.2) VisIt -->
<!-- package](https://visit-dav.github.io/visit-website/releases-as-tables/#latest){target="_blank"} inside an -->
<!-- Ubuntu container. -->

```sh
mkdir /visit   # create a top-level directory inside the sandbox
cd /visit
wget https://github.com/visit-dav/visit/releases/download/v3.4.2/visit3_4_2.linux-x86_64-ubuntu24.tar.gz
tar xvfz visit3_4_2.linux-x86_64-ubuntu24.tar.gz
/bin/rm visit3_4_2.linux-x86_64-ubuntu24.tar.gz
```

4. Convert the sandbox into a SIF image:

```sh
apptainer build visit.sif ubuntu.dir    # creates a 590M SIF image
/bin/rm -rf ubuntu.dir
```

5. Copy the dataset `sineEnvelope.nc` from `/project/def-sponsor00/shared/paraview.zip` into a `~/data`
   folder:

```sh
unzip /project/def-sponsor00/shared/paraview.zip -d ~ data/sineEnvelope.nc
```

6. Start VisIt's Python shell inside the container:

```sh
apptainer exec visit.sif /visit/visit3_4_2.linux-x86_64/bin/visit -cli -nowin
```

7. Paste the following visualization script:

```py
OpenDatabase("~/data/sineEnvelope.nc")
AddPlot("Pseudocolor", "density")

v = GetView3D()                 # get current view attributes
from math import *
phi = 0.3     # 0 <= phi <= 2*pi
theta = 0.3   # -pi/2 <= theta <= pi/2
v.viewNormal = (cos(theta)*cos(phi),cos(theta)*sin(phi),sin(theta))
SetView3D(v)

a = GetAnnotationAttributes()   # get current  annotation attributes
a.axes3D.xAxis.title.font.scale = 0.2  # smaller = smaller text
a.axes3D.xAxis.label.font.scale = 0.2
a.axes3D.yAxis.title.font.scale = 0.2
a.axes3D.yAxis.label.font.scale = 0.2
a.axes3D.zAxis.title.font.scale = 0.2
a.axes3D.zAxis.label.font.scale = 0.2
...
SaveWindow()
```
9. check the resulting image `test*.png`
