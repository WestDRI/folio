...
[NVIDIA NGC](https://catalog.ngc.nvidia.com){target="_blank"} (NVIDIA's hub for GPU-optimized software)
provides prebuilt containers for a large number of HPC applications. Try searching for TensorFlow, GAMESS
(quantum chemistry), GROMACS and NAMD (molecular dynamics), VMD, ParaView, NVIDIA IndeX (visualization). Their
GPU-accelerated containers are quite large, so it might take a while to build them, e.g.
...
### Chapel GPU container demo

::: {.callout-caution}
##### almalinux.sif
I seem to have deleted this container, so I would need to build a new one for this demo. You can
find the instructions for building it [here](../cloud2hpc){target="_blank"}, on a real or virtual machine with
a GPU (and NVIDIA drivers and a CUDA development kit) and Apptainer installed.
:::
...
- the container `almalinux.sif` can be used only on one node (no multi-node parallelism), but it does support
  multiple GPUs
- installed Chapel into an overlay image `chapel.img` (we'll study overlays below)

Here is how I would use it on Fir:
...
apptainer shell --nv -B $SLURM_TMPDIR --overlay $SRC/chapel.img:ro $SRC/almalinux.sif
...
- ![](/fig/no.png) limited to a single node ...\
- ![](/fig/yes.png) no need to adapt container's MPI to the host; just install SSH into the container\
- ![](/fig/yes.png) can build a generic container that will work across multiple HPC clusters (each with a different setup)
...
- ![](/fig/yes.png) can span multiple nodes\
- ![](/fig/no.png) container's MPI should be configured to support the same process management mechanism and
  version (e.g. PMI2 / PMIx) as the host -- not that difficult with a little bit of technical knowledge (reach
  to us for help)
...
<!-- scp mpi.sif user01@apptainer.vastcloud.org:/project/def-sponsor00/shared -->
<!-- ``` -->

```sh
cd ~/tmp
module load openmpi apptainer
unzip -j /project/def-sponsor00/shared/introHPC.zip codes/distributedPi.c

# need to set a couple of things
mkdir -p ~/.openmpi
echo "btl_vader_single_copy_mechanism=none" >> ~/.openmpi/mca-params.conf   # shared-memory comm setting
export PMIX_MCA_psec=native   # allow mpirun inside the container to use host's PMI

export CONTAINER=/project/def-sponsor00/shared/mpi.sif
apptainer exec $CONTAINER mpicc -O2 distributedPi.c -o distributedPi

salloc --ntasks=4 --time=0:5:0 --mem-per-cpu=3600

# try running on the the host
mpirun -np $SLURM_NTASKS ./distributedPi   # error: compiled for Ubuntu, cannot run on Rocky Linux

# try running inside the contaner
mpirun -np $SLURM_NTASKS apptainer exec $CONTAINER ./distributedPi
srun apptainer exec $CONTAINER ./distributedPi   # same as previous line
```
...
[These instructions](/wrf-container.html){target="_blank"} describe building a WRF Apptainer image following
[this build script](https://github.com/Hos128/WRF-CMAQ-Installation){target="_blank"}. This container is large
(8.1GB compressed SIF file, 47GB uncompressed sandbox) and includes everything but the kitchen sink, including
multiple perl and Python 3 libraries and 3rd-party packages. It was created for a support ticket, but what's
important for us is that it also installs MPICH entirely inside the container, not relying on host's
OpenMPI. This means that we'll be limited to MPI runs on one node.
...
du -h sparse.img                   # its actual size is much smaller (316K)
```

Let's mount it and fill with some data, e.g. a few files:
...
Installing native Anaconda on HPC clusters is a [bad
idea](https://docs.alliancecan.ca/wiki/Anaconda/en){target="_blank"} for a number of reasons. Instead of
Conda, we recommend using `virtualenv` together with our pre-compiled Python wheels to install Python packages
into your own virtual environments.
...
Apptainer> df -kh .   # uses ~840M once finished, but more was used during installation
```

These 32k+ files appear as a single file to the Lustre metadata server (which is great!).
...
Of all clusters in the Alliance only Fir has Internet access from compute nodes -- this might limit your
...
<build the container in this directory>  # run on Fir if docker:// access is needed
