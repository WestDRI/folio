...
Similarly, on the training cluster I used `uv` to install free-threaded Python 3.14t into world-readable
`/project/def-sponsor00/shared` (keep in mind that `uv` is [currently not supported on our
clusters](https://docs.alliancecan.ca/wiki/Uv){target="_blank"} so use with caution):
...
launch new threads. Here is a full example of the slow series calculation (store it as `sum.py`) that instead
uses `ThreadPoolExecutor()` function from `concurrent` library:
...
If you run it with v3.12 or earlier, you will get about the same runtime independently of the number of
...
### Alternative: NumExpr (for regular Python)
