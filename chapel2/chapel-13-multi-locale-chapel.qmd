...
Chapel environment is configured properly with the launcher for the cluster's physical interconnect, then you
would simply compile the code and use the launcher binary `mybinary` to run a multi-locale code.
...
Making `srun` work with multi-locale Chapel is a little bit tricky, as both `srun` and multi-locale Chapel
supply their own parallel launcher. For this reason, here we switch from using `srun` to using `sbatch`, which
is also a proper way to launch larger parallel jobs on a producton cluster.
...
::: {.callout-note}
##### Discussion
1. How many Unix processes (MPI ranks) are we launching here?
1. How many CPU cores we'll be using in this job?
:::
...
Before compiling codes, we need to switch from single- to multi-locale Chapel:

```sh
module unload chapel-multicore
