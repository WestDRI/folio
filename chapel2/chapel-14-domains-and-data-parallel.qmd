...
perimeter of *ghost points* if we specify the following (and *if* we have inner blocks in our decomposition):
...
Assuming we started our job with `--nodes=3 --cpus-per-task=2`, let us count the total number of threads
participating in our run by adding the following to our code:
...
[Chapel's documentation on
distributions](https://chapel-lang.org/docs/primers/distributions.html){target="_blank"}. I would particularly
advise to check out
[StencilDist](https://chapel-lang.org/docs/modules/dists/StencilDist.html){target="_blank"} which is an
enhanced variant of the blockDist distribution: it reduces the amount of communication necessary for array
accesses across locale boundaries on a grid.
