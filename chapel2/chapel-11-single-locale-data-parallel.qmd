...
In this code we update all elements of the array `A`. If run on a single node, this code will lauch as many
...
```sh
make forall
srun --mem-per-cpu=3600 --cpus-per-task=4 ./forall
```
```output
count = 500500
```
...
```
Time your code for $n=10^8$, $10^9$ and $10^{10}$ terms.
:::
