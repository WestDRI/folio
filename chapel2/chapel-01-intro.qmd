...
## Language for large-scale parallel computing
...
- Supports **three levels** of hardware parallelism.
...
## Running Chapel codes on production clusters

On Alliance's production clusters Fir / Nibi / Rorqual / Narval we have three versions of Chapel:

1. single-locale (single-node) `chapel-multicore`
1. multi-locale `chapel-ucx` for InfiniBand clusters (all newer clusters)
1. `chapel-ucx-cuda` for InfiniBand clusters with GPU support

<!-- ::: {.callout-note} -->
<!-- We also keep an older multi-locale version `chapel-ofi` for OmniPath clusters for internal use on the old -->
<!-- Cedar hardware at SFU. -->
<!-- ::: -->

You can find the documentation on running Chapel in
[our wiki](https://docs.alliancecan.ca/wiki/Chapel){target="_blank"}.
...
## Running Chapel in Windows

In Windows, if you are familiar with [Docker](https://chapel-lang.org/install-docker.html){target="_blank"}
and have it installed, you can run [multi-locale Chapel inside a Docker
container](https://hub.docker.com/r/chapel/chapel){target="_blank"}:
...
You can also run Chapel under the Windows Subsystem for Linux (WSL) as explained in [this
post](https://chapel-lang.org/blog/posts/nvidia-gpu-wsl){target="_blank"}.

## Running Chapel in MacOS
...
::: {.callout-caution}
## Training cluster
Now we will distribute the usernames and passwords. Once you have these, we will log in to the training
...
login node with CPU-intensive tasks. Therefore, we should be running test Chapel codes inside submitted jobs
on cluster's compute nodes. There are three possible workflows:

#### 1. We can write a job script `serial.sh`
...
#### 2. Alternatively, we could work inside a long serial interactive job

- <span style="color: green;">**pro**:</span> code starts right away, output goes into the terminal
- <span style="color: red;">**con**:</span> most of the time you'll be sitting on allocated resources and not
  actively using them
...
#### 3. Perhaps, the best compromise is to submit short interactive jobs

- <span style="color: green;">**pro**:</span> efficient use of resources, output goes into the terminal

```sh
chpl test.chpl
srun --mem-per-cpu=3600 ./test
```
