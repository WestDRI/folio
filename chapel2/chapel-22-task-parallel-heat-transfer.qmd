...
Now we'll parallelize our heat transfer solver. Let's copy `baseSolver.chpl` to `taskParallelSolver.chpl` and then
start editing the latter. We'll make the following changes in `taskParallelSolver.chpl`:

```
diff baseSolver.chpl taskParallelSolver.chpl
18a19,24
...
```sh
chpl --fast taskParallelSolver.chpl
sed -i -e 's|baseSolver|taskParallelSolver|' shared.sh
sbatch shared.sh
cat solution.out
```
```output
...
Let's copy `taskParallelSolver.chpl` to `taskParallelSolver2.chpl` and then start editing the latter. We'll make
...
Now let's compare the performance of `taskParallelSolver2.chpl` to the benchmark serial solution
`baseSolver.chpl`:

```sh
sed -i -e 's|taskParallelSolver|baseSolver|' shared.sh
sbatch shared.sh
cat solution.out
```
```output
...
```sh
chpl --fast taskParallelSolver2.chpl
sed -i -e 's|baseSolver|taskParallelSolver2|' shared.sh
sbatch shared.sh
cat solution.out
```
```output
...
```sh
./taskParallelSolver2 ... --rowtasks=1 --coltasks=1   # 32.2201 seconds
./taskParallelSolver2 ... --rowtasks=2 --coltasks=2   # 10.197 seconds
./taskParallelSolver2 ... --rowtasks=4 --coltasks=4   # 3.79577 seconds
./taskParallelSolver2 ... --rowtasks=4 --coltasks=8   # 2.4874 seconds
```
