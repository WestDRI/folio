...
::: {.callout-note}
##### C types
Note the types returned by `pixel(z0)` and the type of `stability`.
:::
...
of pixels to a PNG file. The following code, when added to `juliaSetSerial.chpl`, writes the array `stability`
to a file `2000.png`:

```chpl
// add this snippet to juliaSetSerial.chpl
writeln("Plotting ...");
...
To make it work, you also need to:

1. download the [colour map in CSV](/files/nipy_spectral.csv){target="_blank"} and
2. add the file `sciplot.chpl` (below) to your working directory
