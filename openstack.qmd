...
format:
  html:
    include-in-header: schemas/openstack-schemas.html
---

**Tuesday, May 5<sup>th</sup>, 2026**
...
The Alliance Federation provides free cloud resources to researchers on several national systems -- Arbutus,
Cedar Cloud, Nibi Cloud, and Béluga Cloud -- the up-to-date list of which you will always find at
...
- [Arbutus v2.0](https://arbutus.alliancecan.ca){target="_blank"} is now available, and there is a handy
  [Arbutus Migration Guide](https://docs.alliancecan.ca/wiki/Arbutus_Migration_Guide){target="_blank"}
- [Cedar Cloud](https://cedar.cloud.alliancecan.ca){target="_blank"} will eventually be replaced by Fir Cloud
...
On your own computer use your favourite Python package manager, e.g.
...
uv pip install keystoneauth-websso     # needed for SSO (Single Sign-On) on Arbutus2
# uv pip install python-manilaclient   # optional, to programmatically manage shared file systems
...
deactivate
```
...
::: {.callout-note}
##### OpenStack command syntax
You can use one of two formats: either type full bash commands
```sh
$ openstack configuration show
$ openstack flavor list
```
or enter the OpenStack shell to save keystrokes
```sh
$ openstack
(Cmd) configuration show
(Cmd) flavor list
```
:::
...
1. Log into [Cedar Cloud](https://cedar.cloud.alliancecan.ca){target="_blank"} or [new
   Arbutus](https://arbutus.alliancecan.ca){target="_blank"}
1. In the left-side drop-down menu, select your project
<!-- CCInternal-SFU-training -->   <!-- CCInternal-WG-Supp or CCInternal-magic-castle-training project -->
1. In the right-side drop-down menu, click "Download OpenStack RC File"
    - bash script with all env variables to access your OpenStack project through the command-line tools
	- RC = Run Commands
1. On the new Arbutus only: edit the RC file following [these
   instructions](https://docs.alliancecan.ca/wiki/Arbutus_Migration_Guide#New_Arbutus_RC_File_Modifications){target="_blank"}
...
and then either enter your CCDB password at the prompt (Cedar Cloud), or authenticate via the web browser
(Arbutus).

At any point after this, you can check which cloud system you are authenticated against with:

```sh
openstack configuration show | grep -e auth_type -e auth_url -e auth.project_name
```
...
and disk size. There are two types of flavours in our cloud, not counting the GPU flavours:
...
|:-------|:-------|:------|:------|
| Cedar  | c2-15gb-31 | Ubuntu-24.04-Noble-x64-2026-03 | CCInternal-SFU-training-network |
| Arbutus | p2-3gb | Ubuntu-25.04-x64-2025-08 | CCInternal-magic-castle-training-network |
:::
...
sudo apt install -y emacs-nox   # emacs without X11
```
...
patches:
```sh
sudo unattended-upgrade -d   # install only security patches; will take a while (~9m)
sudo shutdown -r now         # reboot; only if /var/run/reboot-required is present
sudo apt upgrade -y          # upgrade installed software to their latest versions
```
...
Alternatively, we can run a VNC server on our VM:

```sh
sudo apt update
sudo apt install -y xfce4 xfce4-goodies   # lightweight desktop environment; takes ~6m
sudo apt install -y tigervnc-standalone-server tigervnc-common   # fast
sudo apt-get install -y libxkbcommon-x11-0 libxcb-icccm4 libxcb-image0 libxcb-keysyms1
sudo apt-get install -y libxcb-render-util0 libxcb-xinerama0 libxcb-xinput0 libxcb-shape0

vncpasswd   # create a new VNC password
mkdir -p ~/.vnc
...
vncserver :1 -geometry 1920x1080 -depth 24   # start the VNC server on port 5901
```

For security, port 5901 is closed on the cloud system's firewall, so we'll use SSH port forwarding on our own
machine:

```sh
ssh ubuntu -L 5900:localhost:5901
```

Start TigerVNC client and connect to `localhost:5900` (typing the VNC password you set up few minutes ago) --
this should open a remote desktop on our VM.
...
openstack server remove security group alexDemoBox rule01   # detach the security group from our instance
