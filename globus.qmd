...
::: {.callout-note}
Creating guest collections on a Globus Connect Personal mapped collection is supported, but it requires a
subscription (typically a paid service). In short, by default you cannot share files hosted on your own
computer. See details
...
project002`, but they cannot see the files inside it. For file access, give your collaborator permission (`r`
...
and transfer files (download with `r` and download/upload with `rw`) with `globus transfer ...` or via the web
interface.
