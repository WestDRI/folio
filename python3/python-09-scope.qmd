...
::: {.callout-note}
You can also implement this code setting `payment = None` for a missing variable, and then placing the code
under `if payment == None` condition. Alternatively, you can put everything into a function with optional
arguments that default to None unless assigned values. I personally like the implementation with the exception
handling, as this way you don't have to assign the missing variable at all.
:::
...
::: {.callout-caution collapse="true"}
## Check your solution
0.1 in binary will be 0.0001(1001) which we then truncate with round-off, then perform arithmetic, then
convert back to decimal with round-off.
:::

::: {.callout-caution collapse="true"}
## More challenging floating-point exercise
