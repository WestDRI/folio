...
::: {.callout-note}
The two macros `@btime` and `@distributed` do not work nicely with each other. If you try to replace
...
around 150ms on my computer with 4 workers.
:::
...
I tested this code (`parallelFor.jl`) on the old Cedar cluster with v1.5.2 and `n=Int64(1e9)`:
...
You can use anonymous (lambda) functions inside `pmap()`:
...
```sh
    ...
    From worker 4:	  0.119988 seconds
    From worker 2:	  0.122788 seconds
    From worker 3:	  0.137920 seconds
    From worker 5:	  0.140048 seconds
