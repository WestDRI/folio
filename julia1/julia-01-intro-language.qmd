...
If you have Julia installed on your own computer, you can run it there: on a multi-core laptop/desktop you can
launch multiple threads and processes and run them in parallel.

If you want to install Julia after this workshop, you can find it [here](https://julialang.org/downloads){target="_blank"}.

## Using Julia on supercomputers

### Julia on the Alliance production clusters

Julia is among hundreds of software packages installed on the Alliance clusters. To use Julia on one of them,
you would load the following module:
...
::: {.callout-note}
Some Julia packages rely on precompiled bits that developers think would work on all architectures,
...
We have Julia on our training cluster *cass.vastcloud.org*.

<!-- node   = { type = "c4-15gb-83", tags = ["node"], count = 10 }   # 17 for the full-scale cluster -->

::: {.callout-note}
##### Our training cluster
<ul>
<li> one fairly small login node,</li>
<li> 10 compute nodes with 4 cores and 15GB of memory on each &nbsp;⇨&nbsp; 3.75GB/core, 40 cores in total</li>
</ul>
:::
...
virtualized hardware with a shared filesystem. If many workshop participants start installing packages at the
same time, this will hammer the filesystem and will make it slow for all participants for quite a while.
...
```sh
$ salloc --mem-per-cpu=3600 --cpus-per-task=2 --time=2:0:0
```
...
training cluster only has ~40 compute cores, so if we use `--cpus-per-task=4` some of us could be left waiting
...
Julia uses
[just-in-time compilation](https://en.wikipedia.org/wiki/Just-in-time_compilation){target="_blank"}
or JIT based on
[LLVM](https://en.wikipedia.org/wiki/LLVM){target="_blank"}:
the source code is compiled at run time. This
combines the flexibility of interpretation with the speed of compilation, bringing speed to an interactive
language. It also allows for dynamic recompilation, continuous weighing of gains and costs of the compilation
of parts of the code, and other on the fly optimizations.

[Here](https://kipp.ly/blog/jits-intro){target="_blank"} is a great blog post covering this topic if you want
to dive deeper into the functioning of JIT compilers.

### Macros

In the tradition of Lisp, Julia has [strong metaprogramming capabilities](https://en.wikibooks.org/wiki/Introducing_Julia/Metaprogramming#Macros){target="_blank"}, in particular in the form of macros.

Macros are parsed and evaluated first, then their output gets evaluated like a classic expression. This allows
the language to modify itself in a
[reflective](https://en.wikipedia.org/wiki/Reflective_programming){target="_blank"} manner.
...
[Julia supports unicode](https://docs.julialang.org/en/v1/manual/unicode-input){target="_blank"}. In a Julia REPL,
...
Our beginner's
[introduction to Julia course](https://rcmodules22.netlify.app/programming_julia){target="_blank"}
has, amongst others, sections on:

- [working with packages](https://westgrid-julia.netlify.app/2022_modules/06_jl_pkg){target="_blank"}
- [working with functions](https://westgrid-julia.netlify.app/2022_modules/08_jl_functions){target="_blank"}
- [control flow](https://westgrid-julia.netlify.app/2022_modules/09_jl_control_flow){target="_blank"}
- [arrays](https://westgrid-julia.netlify.app/2022_modules/13_jl_arrays){target="_blank"}
