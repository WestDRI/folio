...
::: {.callout-important}
Today we are working on a compute node inside an interactive job scheduled with `salloc`. Do not run
...
::: {.callout-note}
##### Confusion with `threadid())` reporting
In a multi-threaded task, recent versions of Julia start thread counting for *worker* threads from 2 (the
main/interactive thread still being 1), whereas the earlier versions were starting counting worker threads
from 1.
:::
...
::: {.callout-note}
`@threads` macro is well-suited for shared-memory data parallelism without any reduction. Curiously,
...
On my computer in three runs I get 0.670s, 0.644s, 0.609s.
...
-- on the same computer I see 0.375s, 0.372s, 0.422s. If I reschedule the job with `--cpus-per-task=4` and use
`julia -t 4`, the times change to 0.266s, 0.202s, 0.225s.

::: {.callout-note}
##### More accurate timing
There is also `@btime` from BenchmarkTools package that has several advantages over `@time`. We
...
On my computer I get 26.73s, 26.84s, 26.58s. Here `@time` also includes JIT compilation time (marginal
here). Let's switch to `@btime` from BenchmarkTools: it runs the code several times, reports the shortest
time, and prints the result only once. Therefore, with `@btime` you don't need to precompile the code.

In this example, to get the correct `total`, we'll have to significantly modify our code:

```jl
using BenchmarkTools
n = Int64(1e8)
@btime begin
    global total = Int128(0)   # 128-bit for the result!
    for i in 1:n
	    global total += i
    end
end
...
```
```sh
5.749s - quite different from the previous numbers
```
...
@btime slow(Int64(1e8), 9)   # total = 13.277605949858103, runtime 518ms
```
...
With one thread I measured 653ms. The runtime stayed essentially the same (now we are using `atomic_add()`)
...
**Update 2**: in recent Julia versions, false sharing has been significantly minimized. Here are the timings
from two successive calls to `slow()` and `space()` on my computer:
...
```sh
[~/tmp]$ julia separateSums.jl
  525.799 ms (9 allocations: 496 bytes)
  526.418 ms (9 allocations: 576 bytes)
[~/tmp]$ julia -t 2 separateSums.jl
  274.016 ms (14 allocations: 896 bytes)
  270.299 ms (14 allocations: 1.02 KiB)
[~/tmp]$ julia -t 4 separateSums.jl
  143.470 ms (24 allocations: 1.62 KiB)
  139.236 ms (24 allocations: 1.91 KiB)
```
...
Let's time this version together with `heavyThreads.jl`: 524.122 ms → 274.436 ms → 141.827 ms when going 1
core → 2 cores → 4 cores.
...
threadid()                  # always shows 1 = local/interactive thread
...
n = Int64(1e8)
@time slow(n, 9, 1, n, 1)    # run the code in serial (one interval, use one thread)
@time slow(n, 9, 1, n, 4)    # 4 intervals, all but one spawned on one of `nthreads()` threads
@time slow(n, 9, 1, n, 16)   # 16 intervals, better saturation of threads,
                             # all but one spawned on one of `nthreads()` threads
```
