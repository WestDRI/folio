...
Let's run this code with `julia juliaSetSerial.jl`. On my laptop it reports 659ms.

::: {.callout-note}
Without the two `const` keywords the code will slow down to 1.669s on the same laptop. This shows the
...
::: {.callout-note}
Built-in plotting in Julia is fairly slow and will take forever for drawing much larger fractals
(e.g. $8000^2$). A faster alternative is to save your plot as compressed NetCDF and visualize it with
something faster, e.g. ParaView. The code for this is below.
:::
...
3. Increase the problem size from the default $2000^2$. Will you have enough physical memory for $8000^2$?
...
On my laptop with 8 threads the timing is 126ms (5.2X speedup) which is good but not great -- certainly worse
than a linear speedup ... The speedup on the training cluster is not great either. There could be several
potential problems:
...
multiple cores, we would love to see your solution! Please only check the
[solution](/badSpeedupSolution.html){target="_blank"}
once you have worked on the problem yourself.
:::
...
You can also generate a truly 3D fractal in Julia. Check out
[this dataset](https://ccvis.netlify.app/gallery/mandelbulb){target="_blank"}
which was computed in Julia and visualized in ParaView (both on a laptop).
