...
@btime sum(!digitsin(9, i) ? 1.0/i : 0 for i in 1:100_000_000)
   # serial code: 553ms, prints 13.277605949858103
```
...
@btime ThreadsX.sum(!digitsin(9, i) ? 1.0/i : 0 for i in 1:100_000_000)
   # with 4 threads: 135ms, prints 13.277605949854381
```

::: {.callout-caution collapse="true"}
##### Exercise ThreadsX.2
The expression `[i for i in 1:10 if i%2==1]` produces an array of odd integers between 1 and 10. Using this
syntax, remove terms that are zero from the last generator, i.e. write a parallel code for summing the slow series
...
@btime ThreadsX.sum(numericTerm, 1:Int64(1e8))   # 135ms, same result
```
...
@btime ThreadsX.maximum(r)   # 37ms with 4 threads
```
