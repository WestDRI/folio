...
format:
  html:
    include-in-header: schemas/chapel-gpu-schemas.html
---

**April 30<sup>th</sup>, 10:00am-noon Pacific Time**

<!-- **To do**: -->
<!-- 1. check if complex numbers are now supported on a GPU, for `juliaSetGPU.chpl` -->
<!-- 2. try plotting on the GPU -->
...
backend](https://llvm.org/docs/NVPTXUsage.html){target="_blank"} to support GPU programming, so usually you
cannot use the system-provided LLVM module -- instead you should set `CHPL_LLVM=bundled` during Chapel
compilation.

As of this writing, on the Alliance systems, you can use GPUs from Chapel on the following systems:

1. native multi-locale on Fir, Rorqual, Narval; the binaries might not be in place everywhere, so
   please get in touch if you want to run it on a specific system
1. native single-locale on Nibi
...
If you run **Linux** and have an NVIDIA GPU on your computer and all the right GPU drivers and CUDA installed,
it should be fairly straightforward to compile Chapel with GPU support. Here is [what worked for me in
AlmaLinux 9.4](https://gist.github.com/razoumov/03cfc54cc388675389bb4343beb8a6b1){target="_blank"}. Please let
me know if these steps do not work for you.

In **Windows**, Chapel with GPU support works under the Windows Subsystem for Linux (WSL) as explained in
[this post](https://chapel-lang.org/blog/posts/nvidia-gpu-wsl){target="_blank"}. You *could* also run Chapel
inside a [Docker container](https://chapel-lang.org/install-docker.html){target="_blank"}, although you need
to find a GPU-enabled Docker image.
...
- for code development and debugging on MacOS you can emulate GPUs, running any GPU Chapel code in the
  *CPU-as-device* mode
...
1. **Chapel with GPU support compiled for NVIDIA cards**. On the training cluster we have 10 GPU nodes, and on
   each node one A100 GPU is split into three "2g.10gb" MIGs, for the total of 30 slices that you can schedule
   with Slurm. To load this version of Chapel, run:
...
<!-- We have 1 virtual GPU on the training cluster to -->
...
2. We could also run Chapel GPU code in the so-called **'CPU-as-device' mode** that emulates a GPU on a
   ...
   [here](https://chapel-lang.org/docs/technotes/gpu.html#cpu-as-device-mode){target="_blank"}. Since we have
   enough physical GPUs today, I did not enable this mode.
   
<!-- To enable this mode, I -->
...
So how do you actually run code on GPU nodes? On the training cluster, to keep resources from sitting idle,
GPU nodes are created on demand: they spin up when a job starts and shut down soon after it finishes. Bringing
a node online usually takes around 3-4 minutes.

We can adopt one of three workflows:

1. use a long interactive job
    - <span style="color: green;">**pro**:</span> code starts right away, output goes into the terminal
    - <span style="color: red;">**con**:</span> you often sit on allocated resources even when you are not actively using them

<!-- --nodelist=nodegpupool2 -->
...
2. submit short batch jobs
    - <span style="color: green;">**pro**:</span> efficient use of resources
    - <span style="color: red;">**con**:</span> output goes to a file
...
3. submit short interactive jobs
    - <span style="color: green;">**pro**:</span> efficient use of resources, output goes into the terminal
...
Let's take a look at the example from
<https://chapel-lang.org/blog/posts/intro-to-gpus>{target="_blank"}. They define a function:
...
More on `@assertOnGpu` and other attributes at
<https://chapel-lang.org/docs/main/modules/standard/GPU.html>{target="_blank"}.
...
simultaneous threads that can finish at different times, so measuring the wallclock time per thread might not
be very meaningful. However, you can measure GPU clock cycles spent on a partucular part of the kernel
function. The `GPU` module provides a function `gpuClock()` that returns the clock cycle counter (per
multiprocessor), and it needs to be called to time code blocks *within a GPU-enabled loop*.
...
Here is a simple example (modelled after
[measureGpuCycles.chpl](https://raw.githubusercontent.com/chapel-lang/chapel/main/test/gpu/native/measureGpuCycles.chpl){target="_blank"})
to demonstrate its use. In this example, we have a parallel loop with `n=10` iterations, each likely launching
a large array of threads, all running at the same time. We count the number of GPU clock cycles per each
iteration.

<!-- This is not the most efficient code, as on the GPU we are parallelizing the loop with -->
<!-- `n=10` iterations, each launching its own thread, and then inside each iteration we run a serial loop to keep -->
<!-- the GPU cores busy, but it gives you an idea. -->
...
::: {.callout-note}
###### Measuring wallclock time on the CPU
...
A = 35663.5 35667.2 35670.7 35674.1 35677.4 35680.6 35683.8 35686.9 35689.9 35692.9
```
We get the same numerical results, but these runtimes make more sense, so it appears that
`gpuClocksPerSec(devNumber)` actually measures the number of GPU clock cycles per *millisecond*.
:::
...
please see [this page](https://chapel-lang.org/docs/main/modules/standard/GPU.html){target="_blank"}.
...
<https://chapel-lang.org/blog/posts/gpu-data-movement>{target="_blank"} -- let's store this file as
`primesGPU-distributed.chpl`:
...
In the code above, the array `A_on_first_host` resides entirely in host's memory on one node. With a
sufficiently large problem, you can distribute `A_on_host` across multiple nodes using block distribution:
...
In the [Julia set problem](https://en.wikipedia.org/wiki/Julia_set){target="_blank"} we need to compute a set
of points on the complex plane that remain bound under infinite recursive transformation $f(z)$. We will use
the traditional form $f(z)=z^2+c$, where $c$ is a complex constant. Here is our algorithm:
...
Chapel's [Image](https://chapel-lang.org/docs/main/modules/packages/Image.html){target="_blank"} library lets
you write arrays of pixels to a PNG file. Here are the steps to implement this output in our code.
...
3. download the [colour map in CSV](/files/nipy_spectral.csv).

When all compiled, this writes the array `stability` to a file `2000.png`, assuming you are using the default
...
- [Introduction to GPU Programming in Chapel](https://chapel-lang.org/blog/posts/intro-to-gpus){target="_blank"}
- [Chapel's High-Level Support for CPU-GPU Data Transfers and Multi-GPU Programming](https://chapel-lang.org/blog/posts/gpu-data-movement){target="_blank"}
- [GPU module functions and attributes](https://chapel-lang.org/docs/main/modules/standard/GPU.html){target="_blank"}
