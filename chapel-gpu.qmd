...
```sh
srun --mem-per-cpu=3600 --gpus-per-node=2g.10gb:1 bash -c "chpl --fast test.chpl && ./test"
```
...
function per thread. The `GPU` module provides a function `gpuClock()` that returns the clock cycle counter
(per multiprocessor), and it needs to be called to time code blocks *within a GPU-enabled loop*.
