...
::: {.callout-note}
Starting with Chapel 2.2, there is an additional attribute `@gpu.assertEligible` that asserts that a
statement is suitable for GPU execution (same as `@assertOnGpu`), without requiring it to be executed on a
GPU. This is perfect in the 'CPU-as-device' mode: fails to compile a `for` loop, but no warnings at runtime.
:::
