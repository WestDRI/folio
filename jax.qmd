...
When you apply a convolutional kernel with `nnx.Conv()`, the model doesn't know what itls supposed to "see"
...
Optax provides a broad range of optimizers, from standard deep learning workhorses to specialized scientific
and memory-efficient algorithms. In Optax, these are essentially "aliases" for pre-packaged chains of gradient
transformations.
...
- `optax.adamw` is the Adam optimizer with weight decay decoupled from the learning rate; weight decay is a
  ...
  thus produce smoother solutions while reducing overfitting
...
## First network: convolutional autoencoder
...
results in a slightly less awful solution:

:::{style="text-align: center;"}
![Initial condition (left), exact solution (center), neural network prediction (right) after 390 epochs](/fig/realUNet.png)
:::

As it turns out, it is actually very common for a standard U-Net to struggle with PDEs. While U-Nets are great
...
## Third network: Fourier Neural Operator (FNO)

A *neural operator* is a mapping between discretized function spaces, e.g.

- in our case: initial condition of a PDE → solution at a fixed later time
- could also map initial condition → time-dependent solution
- easiest: solution at one time step → solution at the next time step
    - this is called an auto-regressive neural net, as we apply an FNO to the output of the previous FNO

Fourier Neural Operator (FNOs) were pioneered by Li et al. in 2020 -- see a diagram below copied from their
original paper.
...
We stack four of these spectral layers in our network:
...
:::{style="text-align: center;"}
![Solution after 10 (left), 100 (center), and 490 (right) epochs](/fig/fno1.png)
:::
