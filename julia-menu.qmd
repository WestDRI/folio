...
format:
  html:
    include-in-header: schemas/julia-menu-schemas.html
---

**March 19<sup>th</sup> (Part 1) and 26<sup>th</sup> (Part 2), 10:00am - noon Pacific Time**
...
also supports memory arrays distributed across multiple processes either on the same or different nodes.

In this hands-on course, we begin with Julia's multithreading features and then move on to the Distributed
multiprocessing standard library and its broad set of tools. We will demo parallelization using two problems:
a slowly converging series and a Julia set. We will run examples on a multi-core laptop and an HPC cluster.
