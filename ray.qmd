...
format:
  html:
    include-in-header: schemas/ray-schemas.html
---
...
- [Ray](https://www.ray.io){target="_blank"} is a unified framework for scaling AI and Python applications.
- [Dask](https://www.dask.org){target="_blank"} parallelizes Python loops and graphs of functions, scales
  NumPy, pandas, and scikit-learn.
- [PySpark](https://spark.apache.org/docs/latest/api/python){target="_blank"} is the Python API for Apache
  Spark for large-scale data processing in a distributed environment.
- [Mars](https://mars-project.readthedocs.io){target="_blank"} is a tensor-based unified framework for
  large-scale data computation which scales NumPy, pandas, scikit-learn and many other libraries.
- [mpi4py](https://mpi4py.readthedocs.io){target="_blank"} is the most popular Message Passing Interface (MPI)
  implementation for Python.
- [IPyParallel](https://ipyparallel.readthedocs.io){target="_blank"} architecture for parallel and distributed
  computing.
- [Joblib](https://joblib.readthedocs.io/en/latest/parallel.html){target="_blank"} for parallel for loops with multiprocessing.
...
## Running on the training cluster

Here is the setup for this part (do this from the **login node**):
...
export RAY_ACCEL_ENV_VAR_OVERRIDE_ON_ZERO=0   # to get rid of a new annoying warning
...
"ray::IDLE"` there. Inside `htop` press “Shift+H” to show/hide individual threads or group them
inside their parent processes.
:::

::: {.callout-caution collapse="true"}
## Question 10a: pass the number of CPU cores to a new Ray cluster
How would you pass the actual number of CPU cores to the Ray cluster? Consider three options:  
...
2. Launching a single-node Ray cluster as described in our
   [Ray documentation](https://docs.alliancecan.ca/wiki/Ray){target="_blank"}.  
3. Not passing anything at all, in which case Ray will start as many workers as there are physical CPU cores
   on the node, i.e. it will guess your runtime environment most likely incorrectly.
:::

::: {.callout-caution collapse="true"}
## Question 10b: get the number of CPU cores in your Ray cluster
Once the Ray cluster is running, which Python construct would you use to get the number of CPU cores in the
current Ray cluster? When you run this contsruct, it should print an integer number of cores.  
**Hint**: look into the information returned by `ray.nodes()`.
:::
...
We used `tqdm.tqdm()` progress bars earlier in the NumPy section. Ray has its own `tqdm()`:
...
@ray.remote
def busy(name):
    if "2" in name: sleep(5)
    for x in tqdm(range(100), desc=name):
        sleep(0.1)

busy.remote("task 1")   # runs write away
busy.remote("task 2")   # sleeps for 5s and then runs
...
::: {.callout-note}
Ray's `tqdm()` is somewhat buggy, so you might want to restart Python and your Ray cluster, if you don't want
to see artifacts from previous bars sometimes popping up in your session.
:::
...
::: {.callout-caution collapse="true"}
## Question 12
Increase `n` to 1_000_000_000 and run `htop --filter "ray` in a separate shell to monitor CPU usage of
...
def runCompiled(x):
    ...
    return compiledSquare(x)

r = runCompiled.remote(5)
ray.get(r)
```

In my tests with more CPU-intensive functions, both versions produce very similar runtimes.

::: {.callout-caution collapse="true"}
## Question 13 (BIG ONE): combining Numba and Ray remotes for the slow series
...
::: {.callout-note}
##### Running time
We have 4 Ray workers (hopefully, running on 4 CPU cores) and 20 tasks, each taking between 1s and 100s, so
it'll likely take 5 tasks per worker and ~250s on average to run.
:::
...
r1, r2, r3 = threeNumbers.remote()
ray.get(r2)   # get second first number only
```
...
By now, you might have noticed that Ray functions (remote tasks) are **stateless**, i.e. they can run on any
processor that happens to be more idle at the time, and they *do not store any data on that processor* in
...
To do this in Ray, we can use [Ray actors](https://docs.ray.io/en/latest/ray-core/actors.html){target="_blank"}. A Ray actor
is essentially a stateful (bound to a CPU core) worker that is created via a Python class instance with its
...
If we want to verify that these arrays stay on the same workers, we can retrieve and print their IDs and
...
general multidimensional array data that are too large to fit into a single machine's memory. In Ray Data,
these large arrays will be:
...
subset of rows, and Ray Data loads and transforms these blocks in parallel.

::: {.callout-important}
## Key point
Each row in Ray's datasets is a dictionary (a collection of key-value pairs).
:::
...
Each row is a dictionary, and dictionaries can have multiple key-value pairs:
...
::: {.callout-note}
This example works on my computer (`pyenv activate env-hpc`), but not on the training cluster where
...
::: {.callout-important}
## Key point
Keep the number of rows in a Ray dataset to a minimum.
:::
...
Can Ray Datasets be *processed with Numba-compiled functions*, similar to how we previously ran Numba-compiled
functions on remote Ray workers? **Absolutely yes**. I will leave it to you to implement this for the slow
series problem. I do have a working solution, but I am not sharing it here because it makes for an excellent
take-home exercise.

A couple of considerations:

1. Numba does not work with Python dictionaries out of the box, instead providing its own dictionary type
   which is not compatible with Ray datasets. You can easily sidestep this problem, but you will have to find
...
::: {.callout-caution collapse="true"}
## Question 15: run as a batch job
...
In our [Ray documentation](https://docs.alliancecan.ca/wiki/Ray){target="_blank"} there are instructions for
launching a single-node Ray cluster. Strictly speaking, this is not necessary, as on a single machine (node) a
call to `ray.init()` will start a new Ray cluster and will automatically connect to it.
...
details of Ray's virtual clusters in the [official Ray
documentation](https://docs.ray.io/en/latest/cluster/getting-started.html){target="_blank"}.

Here we take a look at the example which I copied and adapted from our wiki documentation at
<https://docs.alliancecan.ca/wiki/Ray>{target="_blank"}. We make several changes in this workflow:

1. make it interactive,
2. do not create virtual environments in `$SLURM_TMPDIR` inside the job, but load the existing one from
   `/project/def-sponsor00/shared/env-hpc`,
3. remove GPUs.
...
Then **on each compute node inside our Slurm job, except the head node, we launch the worker nodes** of the
Ray cluster:
...
[here](https://docs.ray.io/en/latest/ray-core/objects/object-spilling.html){target="_blank"}.
...
can find more information at <https://github.com/modin-project/modin>{target="_blank"}.
...
::: {.callout-note}
This example won't work on the training cluster, where `arrow/19.0.1` was compiled without proper
...
## Quickly on parallel Python in ML frameworks
...
- [Single-Program Multi-Data](https://jax.readthedocs.io/en/latest/sharded-computation.html){target="_blank"}
- [Several parallel computing sections in the advanced guide](https://jax.readthedocs.io/en/latest/advanced_guide.html){target="_blank"}

### PyTorch

- [PyTorch Distributed (torch.distributed) Overview](https://pytorch.org/tutorials/beginner/dist_overview.html){target="_blank"}
- [Getting Started with DistributedDataParallel (DDP) module](https://pytorch.org/tutorials/intermediate/ddp_tutorial.html){target="_blank"}
- [Using multiple GPUs with DataParallel](https://pytorch.org/tutorials/beginner/blitz/data_parallel_tutorial.html){target="_blank"}

### TensorFlow

- [Distributed training with TensorFlow](https://www.tensorflow.org/guide/distributed_training){target="_blank"}
- [Multi-GPU and distributed training](https://www.tensorflow.org/guide/keras/distributed_training){target="_blank"}
